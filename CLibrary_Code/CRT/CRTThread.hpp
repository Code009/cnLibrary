...
/*-------------------------------------------------------------------------*/
#ifndef __cnLibrary_CLib_CRT_CRTOS_H__
#define __cnLibrary_CLib_CRT_CRTOS_H__
...
struct TKRuntime::SystemThread
{
	...
	struct tSingleNotification
	{
		...
		void Wait(ufInt64 Duration)noexcept(true){		std::unique_lock Lock(Context->Mutex);	Context->CV.wait_for(Lock,std::chrono::nanoseconds(Duration),WaitFlag);		}
