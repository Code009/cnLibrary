...
//---------------------------------------------------------------------------
sInt32 cnRTL::SysHandleToValue(HANDLE Handle)noexcept(true)
{
#ifdef _WIN64
...
//---------------------------------------------------------------------------
HANDLE cnRTL::SysHandleFromValue(sInt32 Value)noexcept(true)
{
#ifdef _WIN64
...
//---------------------------------------------------------------------------
uChar16 *cnRTL::wtou(wchar_t *Str)noexcept(true)
{
	return reinterpret_cast<uChar16*>(Str);
}
const uChar16 *cnRTL::wtou(const wchar_t *Str)noexcept(true)
{
	return reinterpret_cast<const uChar16*>(Str);
}
//---------------------------------------------------------------------------
wchar_t *cnRTL::utow(uChar16 *Str)noexcept(true)
{
	return reinterpret_cast<wchar_t*>(Str);
}
const wchar_t *cnRTL::utow(const uChar16 *Str)noexcept(true)
{
	return reinterpret_cast<const wchar_t*>(Str);
}
//---------------------------------------------------------------------------
uIntn cTextEncodingConverter_UnicodeToMultiByte::Convert(void *Dest,uIntn DestSize,const void *Src,uIntn SrcSize,uIntn *SrcConvertedSize)noexcept(true)
{
...
//---------------------------------------------------------------------------
uIntn cTextEncodingConverter_MultiByteToUnicode::Convert(void *Dest,uIntn DestSize,const void *Src,uIntn SrcSize,uIntn *SrcConvertedSize)noexcept(true)
{
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
void cWin32CodePageConvertBuffer::CopyUTF8(const uChar8 *Text,uIntn Length)noexcept(true)
{
	Result.SetString(reinterpret_cast<const char*>(Text),Length);
}
//---------------------------------------------------------------------------
void cWin32CodePageConvertBuffer::TranslateWideChar(const wchar_t *Text,uIntn Length)noexcept(true)
{
...
//---------------------------------------------------------------------------
void cWin32CodePageConvertBuffer::TranslateUTF8(const uChar8 *Text,uIntn Length)noexcept(true)
{
...
//---------------------------------------------------------------------------
void cWin32CodePageConvertBuffer::TranslateUTF16(const uChar16 *Text,uIntn Length)noexcept(true)
{
	return TranslateWideChar(utow(Text),Length);
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
cTimeMeter::cTimeMeter()noexcept(true)
{
	Reset();
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void cTimeMeter::Reset(void)noexcept(true)
{
	::QueryPerformanceCounter(&fCheckPoint);
}
//---------------------------------------------------------------------------
double cTimeMeter::Check(void)noexcept(true)
{
	LARGE_INTEGER Now;
	LARGE_INTEGER PF;
...
//---------------------------------------------------------------------------
int cTimeMeter::Check(int Scale)noexcept(true)
{
	LARGE_INTEGER Now;
	LARGE_INTEGER PF;
	::QueryPerformanceCounter(&Now);
