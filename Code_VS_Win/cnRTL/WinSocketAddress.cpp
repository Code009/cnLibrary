...
//---------------------------------------------------------------------------
eiOrdering cnWinRTL::Win32SocketAddressCompare(const SOCKADDR *addr1,const SOCKADDR *addr2)
{
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
cWin32SocketAddress_IPv4::cWin32SocketAddress_IPv4(uInt32 IP,uInt16 Port)
{
...
//---------------------------------------------------------------------------
eiOrdering cWin32SocketAddress_IPv4::Compare(iAddress *Dest)
{
	return cWin32SocketAddress::Compare(Dest);
}
//---------------------------------------------------------------------------
uInt32 cWin32SocketAddress_IPv4::IPv4Address(void)
{
	return ::ntohl(fAddr.sin_addr.s_addr);
}
uInt16 cWin32SocketAddress_IPv4::IPv4Port(void)
{
	return ::ntohs(fAddr.sin_port);
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
iPtr<iSocketAddress> cnWinRTL::Win32CreateCopySocketAddress(const sockaddr *addr,socklen_t addrlen)
{
...
//---------------------------------------------------------------------------
iPtr<iSocketAddress> cnWinRTL::Win32CreateCopySocketAddress(iAddress *Address)
{
...
//---------------------------------------------------------------------------
uIntn cnWinRTL::Win32CopySocketAddress(void *Buffer,uIntn BufferSize,iAddress *Address)
{
	auto SocketAddress=iCast<iSocketAddress>(Address);
	if(SocketAddress!=nullptr){
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
iPtr<iIPv4Address>	cnWinRTL::Win32CreateAddressIPv4(uInt32 IP,uInt16 Port)
{
	return iCreate<cWin32SocketAddress_IPv4>(IP,Port);
}
//---------------------------------------------------------------------------
uInt16 cnWinRTL::Win32QueryAddressFamily(iAddress *Address)
{
	auto SocketAddress=iCast<iSocketAddress>(Address);
	if(SocketAddress!=nullptr){
...
//---------------------------------------------------------------------------
ufInt16 cnWinRTL::Win32QueryAddressSize(iAddress *Address)
{
	auto SocketAddress=iCast<iSocketAddress>(Address);
	if(SocketAddress!=nullptr){
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
cWin32SocketAddressBuffer::cWin32SocketAddressBuffer()
{
...
//---------------------------------------------------------------------------
void cWin32SocketAddressBuffer::Clear(void)
{
	fBuffer.Clear();
}
//---------------------------------------------------------------------------
SOCKADDR* cWin32SocketAddressBuffer::SockAddr(void)const
{
	return static_cast<SOCKADDR*>(fBuffer->Pointer);
}
//---------------------------------------------------------------------------
socklen_t cWin32SocketAddressBuffer::SockAddrLen(void)const
{
	return static_cast<socklen_t>(fBuffer->Length);
}
//---------------------------------------------------------------------------
void cWin32SocketAddressBuffer::SetSockAddrLen(socklen_t len)
{
...
//---------------------------------------------------------------------------
socklen_t cWin32SocketAddressBuffer::GetCapacity(void)const
{
	return static_cast<socklen_t>(fBuffer.GetCapacity());
}
//---------------------------------------------------------------------------
void cWin32SocketAddressBuffer::SetCapacity(socklen_t len)
{
	if(len<sizeof(SOCKADDR))
		len=sizeof(SOCKADDR);
...
//---------------------------------------------------------------------------
void cWin32SocketAddressBuffer::SetupBuffer(ADDRESS_FAMILY sa_family,socklen_t socklen)
{
	auto *saddr=SockAddr();
...
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::SetAddressAny(ADDRESS_FAMILY sa_family)
{
	auto *saddr=SockAddr();
	switch(sa_family){
	default:
...
//---------------------------------------------------------------------------
iPtr<iSocketAddress> cWin32SocketAddressBuffer::CreateSocketAddress(void)
{
...
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::SetupForAddressFamily(ADDRESS_FAMILY sa_family)
{
	auto *saddr=SockAddr();
	switch(sa_family){
	default:
...
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::SetupRecvAddress(const SOCKADDR *addr)
{
	return SetupForAddressFamily(addr->sa_family);
}
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::SetupRecvAddress(iSocketAddress *Address)
{
	return SetupForAddressFamily(Address->SockAddr()->sa_family);
}
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::SetupRecvAddress(iAddress *Address)
{
	auto SocketAddress=iCast<iSocketAddress>(Address);
	if(SocketAddress!=nullptr){
...
//---------------------------------------------------------------------------
const sockaddr* cWin32SocketAddressBuffer::SetupSendAddress(iAddress *Address,socklen_t &addrlen)
{
	auto SocketAddress=iCast<iSocketAddress>(Address);
	if(SocketAddress!=nullptr){
...
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::GetBoundAddress(SOCKET s,ADDRESS_FAMILY sa_family)
{
...
//}
//---------------------------------------------------------------------------
eiOrdering cWin32SocketAddressBuffer::Compare(const SOCKADDR *addr)const
{
	return Win32SocketAddressCompare(static_cast<const sockaddr*>(fBuffer.GetPointer()),addr);
}
//---------------------------------------------------------------------------
bool cWin32SocketAddressBuffer::operator == (const SOCKADDR *addr)const
{
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
cWin32SocketMulticastAddress_IPv4::cWin32SocketMulticastAddress_IPv4(const SOCKADDR_IN *addr,const struct ip_mreq *MulticastAddrs,uIntn MulticastAddrCount)
{
	fAddr=*addr;
	cnMemory::ZeroFill(fAddr.sin_zero);
...
//---------------------------------------------------------------------------
eiOrdering cWin32SocketMulticastAddress_IPv4::Compare(iAddress *Dest)
{
	if(Dest==nullptr)
		return iOrdering::Different;
...
//---------------------------------------------------------------------------
const sockaddr* cWin32SocketMulticastAddress_IPv4::SockAddr(void)
{
	return &fSAddr;
}
//---------------------------------------------------------------------------
socklen_t cWin32SocketMulticastAddress_IPv4::SockAddrLen(void)
{
	return sizeof(fAddr);
}
//---------------------------------------------------------------------------
uIntn cWin32SocketMulticastAddress_IPv4::AddMember(SOCKET s)
{
...
//---------------------------------------------------------------------------
uIntn cWin32SocketMulticastAddress_IPv4::DropMember(SOCKET s)
{
	uIntn SuccessCount=0;
	for(uIntn i=0,c=fMulticastAddressList.GetCount();i<c;i++){
		auto &imr=fMulticastAddressList[i];
...
//---------------------------------------------------------------------------
uInt32 cWin32SocketMulticastAddress_IPv4::IPv4Address(void)
{
	return ntohl(fAddr.sin_addr.s_addr);
}
uInt16 cWin32SocketMulticastAddress_IPv4::IPv4Port(void)
{
	return ntohs(fAddr.sin_port);
}
//---------------------------------------------------------------------------
